...
var easyRender = function(element, options) {
  ...
  if ($.metadata) {
    this.settings =
        $.extend({}, this.settings, element.metadata({type: 'class'}));
    this.settings =
        $.extend({}, this.settings, element.metadata({type: 'html5'}));
  }
  if (element.is('form')) {
    this.form = element;
  } else {
...
easyRender.prototype.beforeSubmit = function() {
  ...
  widget.form.find(widget.settings.loadmore)
...
easyRender.prototype.onSuccess = function(response) {
  ...
  if (widget.settings.json && isJson(response)) {
    displayNoty(response, widget.form);
