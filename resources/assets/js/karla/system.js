function setTask($this, form, task) {
  ...
  if (input.length > 0) {
    ...
    $('<input/>', {name: name, value: task, type: 'hidden'}).appendTo(form);
  }
}
...
function karlaSystemJs() {
  ...
  $(document).on('click', '[data-order]', function(e) {
    ...
    if (form.find('.ac-sort-name').length > 0) {
      ...
      $('<input/>',
        {class: 'ac-sort-name', name: 'sort', value: name, type: 'hidden'})
          .appendTo(form);
    }
    ...
    if (form.find('.ac-sort-order').length > 0) {
      ...
      $('<input/>',
        {class: 'ac-sort-order', name: 'order', value: order, type: 'hidden'})
          .appendTo(form);
    }
  ...
  $(document).on('click', '[role=login]', function(e) {
    ...
    $.fn.easyModalShow({url: url, event: 'ready'});
  });
  ...
  $(document).on('click', '[data-poload]', function(e) {
    ...
    $.get($this.data('poload'), {format: 'html'}, function(d) {
      ...
      $this.popover({content: d, html: true, placement: 'bottom'})
          .popover('show');
